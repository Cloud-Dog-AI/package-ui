...
function tokenToMonacoColor(value) {
...
function resolveThemeColors(theme) {
    ...
    return {
        ...
        "primary-foreground": styles.getPropertyValue("--primary-foreground").trim() || fallback["primary-foreground"],
        secondary: styles.getPropertyValue("--secondary").trim() || fallback.secondary,
        "secondary-foreground": styles.getPropertyValue("--secondary-foreground").trim() || fallback["secondary-foreground"],
        muted: styles.getPropertyValue("--muted").trim() || fallback.muted,
        "muted-foreground": styles.getPropertyValue("--muted-foreground").trim() || fallback["muted-foreground"],
        accent: styles.getPropertyValue("--accent").trim() || fallback.accent,
        "accent-foreground": styles.getPropertyValue("--accent-foreground").trim() || fallback["accent-foreground"],
        destructive: styles.getPropertyValue("--destructive").trim() || fallback.destructive,
        "destructive-foreground": styles.getPropertyValue("--destructive-foreground").trim() || fallback["destructive-foreground"],
...
function defineTheme(monaco, tokenTheme) {
    ...
    monaco.editor.defineTheme(themeName, {
        ...
        colors: {
            ...
            "editor.inactiveSelectionBackground": tokenTheme === "dark" ? "rgba(71,85,105,0.28)" : "rgba(148,163,184,0.22)",
            "editorLineHighlightBackground": tokenTheme === "dark" ? "rgba(148,163,184,0.08)" : "rgba(15,23,42,0.04)",
            ...
            "diffEditor.insertedTextBackground": tokenTheme === "dark" ? "rgba(34,197,94,0.18)" : "rgba(34,197,94,0.12)",
            "diffEditor.removedTextBackground": tokenTheme === "dark" ? "rgba(239,68,68,0.18)" : "rgba(239,68,68,0.12)",
            "diffEditor.insertedLineBackground": tokenTheme === "dark" ? "rgba(34,197,94,0.1)" : "rgba(34,197,94,0.08)",
            "diffEditor.removedLineBackground": tokenTheme === "dark" ? "rgba(239,68,68,0.1)" : "rgba(239,68,68,0.08)",
        },
    });
}
...
export function maskText(value, maskPatterns) {
